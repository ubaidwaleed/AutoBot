...
function DashboardCard4() {
  ...
  const [carCareProducts, setcarCareProducts] = useState(0);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const accessory = data1.data.accessoriesdata.length;
        const part = data2.data.partsdata.length;
        const carCare = data3.data.partsdata.length;

        setParts(part);
        setAccessories(accessory);
        setcarCareProducts(carCare);
    ...
    datasets: [
      {
        ...
        data: [accessories, parts, carCareProducts],
        backgroundColor: [
